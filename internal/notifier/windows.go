...
const windowsToastScript = `function Decode-Hex([string]$Value) {
...
$title = Decode-Hex ([Environment]::GetEnvironmentVariable("SKYTUI_NOTIFICATION_TITLE"))
$message = Decode-Hex ([Environment]::GetEnvironmentVariable("SKYTUI_NOTIFICATION_MESSAGE"))
...
func (w windows) Notify(title, message string) error {
	...
	command := commandSpec{
		...
		environment: []string{
			"SKYTUI_NOTIFICATION_TITLE=" + encodedTitle,
			"SKYTUI_NOTIFICATION_MESSAGE=" + encodedMessage,
		},
	}
